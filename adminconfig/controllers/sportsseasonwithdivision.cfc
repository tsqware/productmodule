...
component persistent="false" accessors="true" output="false" extends="controller" {
	...
	public any function doCreate(required rc) {
		var ent = SportsSeasonWithDivisionsService.new();	
		populateAssociatedValues(rc,ent);		
		SportsSeasonWithDivisionsService.handleInsert(ent, rc);
		
		loadVarsForRedirect(rc);
		
		if (rc.messageStatus eq "success") {
			rc.message = "The Season #rc.seasonName# is now saved.";
			var seasonID = ent.getSeasonID();
			variables.fw.redirect(action="adminconfig:sportsseasonwithdivision.edit", querystring="sea=#seasonID#", preserve="message,messagestatus,valerrors");
		}
		else {
			rc.message = "The Season was not saved.";
			variables.fw.redirect(action="adminconfig:sportsseasonwithleague.create", querystring="prod=#rc.prod#", preserve="all");	
		}
	...
	public any function edit(required rc) {
		...
		if(StructKeyExists(rc, "sea")) {
			rc.thisSeason = SportsSeasonWithDivisionsService.load(rc.sea); // id not param
			if(!IsBoolean(rc.thisSeason)) {
				...
				rc.hasDivision = methodExists(rc.thisSeason, "hasDivision");
				rc.hasPrice = rc.thisSeason.hasPrice();
				rc.products = SportsLeagueRegService.list();
				rc.hasProduct = rc.thisSeason.hasLeagueReg();				

				rc.divisions = SportsSeasonDivisionService.list();
				if(rc.hasDivision) rc.selectedDivisions = rc.thisSeason.getDivisions();
