<cfcomponent extends="EntityDAO">
	...
	<cffunction name="init" access="public" returntype="ProductTypeDAO">
		<cfreturn this />
	</cffunction>
	...
	<cfscript>
		public any function listInProductType(required String producttype) {
			var rows = ORMExecuteQuery(
				"
					SELECT p 
					FROM Product p join p.ProductType pt where pt.productTypeParam = '#producttype#'
				"
			);
			return rows;
		}
		...
		public any function populate(required obj, rc) {
			obj.setProductTypeName(rc.productTypeName);
			obj.setProductTypeParam(rc.productTypeParam);
			obj.setProductTypeClassName(rc.productTypeClassName);					
			return obj;
		}
