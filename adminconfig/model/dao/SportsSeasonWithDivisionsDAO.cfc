<cfcomponent extends="EntityDAO">
	...
	<cffunction name="init" access="public" returntype="SportsSeasonWithDivisionsDAO">
		<cfreturn this />
	</cffunction>
	...
	<cfscript>
		...
		public any function populate(required obj, rc) {
			obj.setSeasonName(rc.seasonName);
			obj.setSeasonParam(rc.seasonParam);
			if(Len(rc.startDate)) obj.setStartDate(rc.startDate);
			else obj.setStartDate(JavaCast('null', ''));

			if(Len(rc.endDate)) obj.setEndDate(rc.endDate);
			else obj.setEndDate(JavaCast('null', ''));

			if(Len(rc.earlyRegDueDate)) obj.setEarlyRegDueDate(rc.earlyRegDueDate);
			else obj.setEarlyRegDueDate(JavaCast('null', ''));

			if(Len(rc.regDueDate)) obj.setRegDueDate(rc.regDueDate);
			else obj.setRegDueDate(JavaCast('null', ''));
