...
component extends="EntityService" {
	public any function init(any prodpricedao) 
	{	
		variables.dao = arguments.prodpricedao;
		return this;
	...
	*/
	...
	public any function loadByParam(String paramname="none") {
		var result = EntityLoad("Price", {priceParam = arguments.paramname},true);
		if (IsNull(result)) {
			return false;
		}
		return result;
	...
	public void function handleInsert(obj, rc) {
		...
		if (ArrayIsEmpty(request.valerrors)) {			
			...
			try {
				if(IsNull(newEnt.getPriceID())) save(newEnt);				
				txn.commit();
			}
			catch(any e) {
				txn.rollback();
