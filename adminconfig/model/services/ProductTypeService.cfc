...
component extends="EntityService" {
	public any function init(any ptdao) 
	{	
		variables.dao = arguments.ptdao;
		return this;
	...
	*/
	...
	public any function loadByParam(String paramname="none") {
		...
		var result = EntityLoad("ProductType", {productTypeParam = arguments.paramname});
		if (IsArray(result) && ArrayLen(result) >= 1)
			result = result[1];
		return result;
	}
	public any function loadByClassName(String classname="none") {
		var result = EntityLoad("ProductType", {productTypeClassName = arguments.classname});
		if (IsArray(result) && ArrayLen(result) >= 1)
			result = result[1];
		return result;
	}		
