<cfcomponent extends="EntityDAO" accessors="true">
	...
	<cffunction name="init" access="public" returntype="PaymentDAO">
		<cfif IsDefined("request.leagueBean")>
			<cfset setSportsLeaguePayment(new pmservices.SportsLeaguePayment(request.leagueBean)) />	
		</cfif>
		<cfreturn this />
	</cffunction>
	...
	***/
	...
	public any function populate(required payment) {
		param name="pricetype" default="payinfull";
		//WriteDump(payment);
		...
		// strategy pattern
		if (form.productType == "Event RSVP") {
			var strat = evaluate("new pmservices.#form.productClassParam#Payment().init()");
			var price = strat.getPrice(); 
		}
		else if (form.productType == "Adult League Product" ) {
			var strat = evaluate("new pmservices.getSportsLeaguePayment().init()");
