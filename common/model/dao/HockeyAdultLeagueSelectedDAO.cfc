<cfcomponent extends="EntityDAO" accessors="true">
	<cfproperty name="SportsLeaguePayment" />
	<cffunction name="init" access="public" returntype="HockeyAdultLeagueSelectedDAO">
		<cfset this.league = request.leagueBean />
		<cfset setSportsLeaguePayment(new pmservices.SportsLeaguePayment(this.league)) />
		<cfset this.leaguePayment = getSportsLeaguePayment() />
		<cfreturn this />
	</cffunction>
	...
	**
	***/
	public void function save(lg) {
		try {
			EntitySave(lg);
		}
		catch(any e) {
			request.message = "There was a problem saving the item selected.";
			request.messagetype = "generror";
	...
	***/
	...
	public any function populate(required lg) {
		//WriteDump(var=this.league.getAllValues(), label="request");
		//abort;
		lg.setProductName(form.productName);
		lg.setProductURLParam(form.productURLParam);
		lg.setProductClassParam(form.productClassParam);
		lg.setProductTypeName(form.productType);
		lg.setLeagueName(this.league.getAdLeagueName());
		lg.setLeagueSeason(this.league.getAdLeagueSeason());
		if (IsDefined("form.divisionName") && Len(Trim(form.divisionName)) > 0) lg.setDivisionName(form.divisionName);
		if (Len(Trim(this.league.getAdLeagueStartDate())) > 0) lg.setLeagueStartDate(this.league.getAdLeagueStartDate());
		if (Len(Trim(this.league.getAdLeagueEndDate())) > 0) lg.setLeagueEndDate(this.league.getAdLeagueEndDate());
		if (Len(Trim(this.leaguePayment.getFullPrice())) > 0) lg.setRegTotal(this.leaguePayment.getFullPrice());
		if (Len(Trim(this.leaguePayment.getBalanceDue())) > 0) lg.setRegBalanceDue(this.leaguePayment.getBalanceDue());
		if (StructKeyExists(form, "comments")) lg.setComments(form.comments);
